...
public class MoveWristToSetpoint extends Command {
  ...
  final TrapezoidProfile profile =
      new TrapezoidProfile(
          new TrapezoidProfile.Constraints(
              frc.robot.utils.constants.WristConstants.Wrist.WRIST_MAX_VELOCITY,
              frc.robot.utils.constants.WristConstants.Wrist.WRIST_MAX_ACCELERATION));
  ...
  public void execute() {
    ...
    positionVoltage.Position =
        positionGoal.position * frc.robot.utils.constants.WristConstants.Wrist.WRIST_GEAR_RATIO;
    wrist.setWristMotorControl(positionVoltage);

    ConditionalSmartDashboard.putNumber(
        "Wrist position goal (motor rotations)", positionGoal.position);
    ConditionalSmartDashboard.putNumber(
        "Wrist velocity goal (motor rotations per second)", positionGoal.velocity);
  }
  ...
  public boolean isFinished() {
    return (Math.abs(wrist.getPosition() - setPoint))
        < frc.robot.utils.constants.WristConstants.Wrist.WRIST_TOLERANCE;
  }
}
